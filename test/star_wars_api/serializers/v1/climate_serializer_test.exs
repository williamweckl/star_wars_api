defmodule StarWarsAPI.V1.ClimateSerializerTest do
  ...
  describe "serialize/1" do
    test "returns serialized record with root key" do
      ...
      assert ClimateSerializer.serialize(record) == %{
               data: %{
                 id: record.id,
                 name: record.name,
    ...
    test "returns serialized records with root key for pagination" do
      ...
      assert ClimateSerializer.serialize(pagination) == %{
               data: [
                 %{
                   id: record_one.id,
  ...
  describe "serialize/2" do
    test "returns serialized record without root key" do
      ...
      assert ClimateSerializer.serialize(record, :without_root_key) == %{
               id: record.id,
               name: record.name,
               inserted_at: record.inserted_at,
               updated_at: record.updated_at
    ...
    test "returns nil" do
      assert ClimateSerializer.serialize(nil, :without_root_key) == nil
    end
  end
end
